...
export class PerformanceOptimizer {
  ...
  async trackResponse(
    operation: string, 
    responseTime: number, 
    category: 'critical' | 'fast' | 'standard' = 'standard'
  ): Promise<void> {
    const now = Date.now();
    ...
    if (responseTime > target) {
      ...
      await cacheService.set(`slow_operation:${operation}:${now}`, {
        operation,
        responseTime,
        target,
        category,
  ...
  static fastProjectionCalculation(parameters: {
    ...
    const riskScore = Math.max(10, Math.min(90, 
      35 + riskFactors.reduce((sum, factor) => sum + factor, 0)
    ));
  ...
  static createTimingMiddleware() {
    ...
    return async (request: any, reply: any, next: any) => {
      ...
      let category: 'critical' | 'fast' | 'standard' = 'standard';
  ...
  static optimizeQuery(baseQuery: string, useIndex: boolean = true): string {
    if (!useIndex) return baseQuery;
