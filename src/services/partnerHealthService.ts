...
import type { PartnerHealthMetric, ApiStatus } from '../types/partner.js';

export class PartnerHealthService {
  constructor(private prisma: PrismaClient) {}

  async monitorPartnerHealth(partnerId: string): Promise<PartnerHealthMetric> {
    ...
    try {
      if (partner.apiCredentials && (partner.apiCredentials as any)?.endpoints?.base) {
        const apiCreds = partner.apiCredentials as any;
    ...
    const healthMetric: PartnerHealthMetric = {
    ...
    await this.prisma.partnerHealthMetric.create({
      data: {
        ...
        details: healthMetric.details as any
      }
    });
  ...
  async batchMonitorHealth(partnerIds?: string[]): Promise<PartnerHealthMetric[]> {
    const partners = await this.prisma.partner.findMany({
      where: partnerIds ? { id: { in: partnerIds } } : { status: 'ACTIVE' },
      select: { id: true }
    });

    const healthChecks = partners.map(partner => 
      this.monitorPartnerHealth(partner.id).catch(error => ({
        timestamp: new Date().toISOString(),
        apiStatus: 'DOWN' as ApiStatus,
        errorRate: 100,
        uptime: 0,
  ...
  async getHealthSummary(partnerId: string, days: number = 7) {
  ...
  async scheduleHealthChecks(intervalMinutes: number = 15) {
  ...
  private async testApiEndpoint(baseUrl: string, credentials: any): Promise<Response> {
  ...
  private calculateHealthScore(responseTime: number, errorRate: number, uptime: number): number {
    // Weight factors
  ...
  private async calculateUptime(partnerId: string, days: number = 1): Promise<number> {
    const since = new Date();
    since.setDate(since.getDate() - days);
  ...
  private determineHealthStatus(lastMetric: any): 'healthy' | 'degraded' | 'down' | 'unknown' {
    if (!lastMetric) return 'unknown';
