...
const PREDEFINED_TEMPLATES: Omit<CampaignTemplate, 'id' | 'usageCount' | 'createdAt' | 'updatedAt'>[] = [
  {
...
export class CampaignTemplatesService {
  ...
  async getPopularTemplates(limit: number = 5): Promise<CampaignTemplate[]> {
  ...
  async getTemplatesByIndustry(industry: string): Promise<CampaignTemplate[]> {
    return this.getAllTemplates({ industry, isActive: true });
  }

  async createCustomTemplate(template: Omit<CampaignTemplate, 'id' | 'usageCount' | 'createdAt' | 'updatedAt'>): Promise<CampaignTemplate> {
  ...
  async getTemplateAnalytics(templateId?: string): Promise<TemplateUsageAnalytics | TemplateUsageAnalytics[]> {
    // Mock analytics data
    const mockAnalytics = (id: string): TemplateUsageAnalytics => ({
      templateId: id,
      usageCount: Math.floor(Math.random() * 500) + 50,
  ...
  async getIndustryBenchmarks(industry: string): Promise<{
    ...
    commonObjectives: Array<{ type: string; frequency: number }>;
    typicalChannelMix: Record<string, number>;
    successMetrics: Array<{ metric: string; averageTarget: number }>;
  }> {
    ...
    return benchmarks[industry as keyof typeof benchmarks] || benchmarks['Consumer Goods'];
  }
  ...
  async recommendTemplates(criteria: {
    ...
    const scoredTemplates = allTemplates.map(template => {
      ...
      if (criteria.objectives && criteria.objectives.length > 0) {
        const matchingObjectives = template.objectives.filter(obj => 
          criteria.objectives!.includes(obj.type)
        );
      ...
      if (criteria.timeline && template.duration.weeks) {
        ...
        if (weeksDiff <= 2) score += 15;
        else if (weeksDiff <= 4) score += 10;
        else if (weeksDiff <= 8) score += 5;
      }
...
export const campaignTemplatesService = new CampaignTemplatesService();