...
const ScenarioParametersSchema = z.object({
  msrp: z.number().min(0.01).max(10000),
  distributorMargin: z.number().min(0).max(100),
  retailerMargin: z.number().min(0).max(100),
  volumeCommitment: z.number().int().min(1).max(10000000),
  marketingSpend: z.number().min(0).max(100000000),
  seasonalAdjustment: z.number().min(0.1).max(5.0),
});
...
export const distributionRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
  ...
  }>('/campaigns/:campaignId/distribution/modeling', {
    schema: {
      params: {
        type: 'object',
        properties: {
    ...
    const cacheKey = cacheService.createDistributionKey(campaignId);
    
    // Use stale-while-revalidate for <1s response
    const session = await cacheService.getStaleWhileRevalidate(
      cacheKey,
      async () => {
  ...
  fastify.patch<{
    Body: z.infer<typeof UpdateParameterSchema>;
  }>('/distribution/scenarios/parameters', {
    schema: {
      body: {
        type: 'object',
        properties: {
    ...
    const parametersCache = await cacheService.get(`session:${sessionId}:scenario:${scenarioId}:params`) || {};
    const updatedParams = { ...parametersCache, [field]: value };
    
    const projectionKey = cacheService.createProjectionKey(sessionId, updatedParams);
    
    // Check cache first for instant response
    let projections = await cacheService.get(projectionKey);
    ...
    if (!projections) {
      ...
      if (isComplexCalculation) {
        ...
        const job = await jobQueue.queueDistributionCalculation({
        ...
        try {
          ...
          projections = (result as any).projections;
        } catch {
      ...
      await cacheService.set(projectionKey, projections, 30);
    }
    
    // Update session cache
    await cacheService.set(`session:${sessionId}:scenario:${scenarioId}:params`, updatedParams, 300);
  ...
  fastify.post<{
    ...
    await cacheService.del(`distribution:${sessionId}:session`);
  ...
  fastify.post<{
    ...
    // Queue export job
    const job = await jobQueue.queuePDFGeneration({
      sessionId,
      campaignId: sessionId,
      format,
  ...
  fastify.get<{
    Params: { jobId: string };
  }>('/distribution/export/:jobId/status', async (request) => {
    const { jobId } = request.params;
    
    try {
      const status = await jobQueue.getJobStatus('pdf', jobId);
      return status;
    } catch (error) {
  ...
  async function quickProjectionEstimate(parameters: Record<string, any>) {
    // Use optimized calculation for <100ms response
    return PerformanceOptimizer.fastProjectionCalculation({
