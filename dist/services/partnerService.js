...
class PartnerService {
    ...
    async updatePartner(id, data, userId) {
        ...
        const partner = await this.prisma.partner.update({
            ...
            data: {
                ...
                ...(data.apiCredentials && { apiCredentials: data.apiCredentials }),
                ...(data.targetingOptions && { targetingOptions: data.targetingOptions }),
                ...(data.measurementCapabilities && { measurementCapabilities: data.measurementCapabilities }),
                ...(data.pricingInfo && { pricingInfo: data.pricingInfo })
            },
            include: {
                capabilities: true,
                creativeSpecs: true,
    ...
    async updatePartnerStatus(id, status, userId) {
        const partner = await this.prisma.partner.update({
            where: { id },
            data: { status },
    ...
    async updateOnboardingStatus(id, status, step, userId) {
        ...
        if (step) {
            ...
            if (partner?.onboardingSteps) {
                const steps = partner.onboardingSteps;
                if (steps.steps) {
                    const stepIndex = steps.steps.findIndex((s) => s.id === step);
                    if (stepIndex !== -1) {
    ...
    async recordHealthMetric(partnerId, metric) {
    ...
    async getPartnerHealth(partnerId, days = 7) {
    ...
    async addCapability(partnerId, capability, userId) {
    ...
    async addCreativeSpec(partnerId, spec, userId) {
    ...
    async associateWithCampaign(association, userId) {
        const result = await this.prisma.campaignPartner.create({
            data: {
                ...
                targeting: association.targeting,
                creative: association.creative
            },
            include: {
    ...
    async generateCapabilityComparison(partnerIds, categories) {
    ...
    generateDefaultOnboardingSteps() {
        return {
            steps: [
                {
    ...
    async logAuditEvent(action, entityId, userId, changes) {
